...
if (isset($_POST["submit"])){
    if ($_POST["zborceneharfyton"]){
        $lemma = $_POST["zborceneharfyton"];
        $conection = mysqli_connect("127.0.0.1", "korpusprozyco001", "prasmoid", "korpusprozycom");
        mysqli_set_charset($conection, "utf8mb4");

        $stmt = $conection->prepare("SELECT id, token FROM zborceneharfytonkonkordance WHERE lemma=?");
        ...
        if ($rowcount > 0){
            while ($radek_lemma = $result_lemma->fetch_assoc()){
                ...
                if ($ID_LEMMA) {
                    ...
                    // PRAVÝ KONTEXT
                    $stmt2 = $conection->prepare("SELECT token FROM zborceneharfytonkonkordance WHERE id=?");
