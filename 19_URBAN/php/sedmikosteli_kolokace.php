...
if (isset($_POST["Submit"])){
    if ($_POST["sedmikosteli"]){
        
        $lemma = $_POST["sedmikosteli"];

        $conection = mysqli_connect("127.0.0.1", "korpusprozyco001", "prasmoid", "korpusprozycom04");
        mysqli_set_charset($conection, "utf8mb4");

        // COUNTING ROWS IN TABLE = N
        $sgl_n = "SELECT id FROM sedmikostelibigramy";
        $result4 = mysqli_query($conection, $sgl_n);
        $N = mysqli_num_rows($result4); //N

        // prave kolokace
        $stmt = $conection->prepare("SELECT DISTINCT LOWER(word2) AS word2 FROM sedmikostelibigramy WHERE word1=?");
        ...
        if ($result_kolokace->num_rows > 0){
            while ($radek_kolokace = $result_kolokace->fetch_assoc()){
                ...
                $stmt1 = $conection->prepare("SELECT COUNT(word1) as w1 FROM sedmikostelibigramy WHERE word2=?");
                ...
                $stmt2 = $conection->prepare("SELECT COUNT(word2) as w2 FROM sedmikostelibigramy WHERE word1=?");
                ...
                if ($result2->num_rows > 0){
                    ...
                    $stmt3 = $conection->prepare("SELECT COUNT(id) as id FROM sedmikostelibigramy WHERE word1=? AND word2=?");
        ...
        // leve kolokace
        $stmt4 = $conection->prepare("SELECT DISTINCT LOWER(word1) AS word1 FROM sedmikostelibigramy WHERE word2=?");
        ...
        if ($result_kolokace2->num_rows > 0){
            while ($radek_kolokace2 = $result_kolokace2->fetch_assoc()){
                ...
                $stmt5 = $conection->prepare("SELECT COUNT(word2) as w2 FROM sedmikostelibigramy WHERE word1=?");
                ...
                $stmt6 = $conection->prepare("SELECT COUNT(word1) as w1 FROM sedmikostelibigramy WHERE word2=?");
                ...
                if ($result5->num_rows > 0){
                    ...
                    $stmt7 = $conection->prepare("SELECT COUNT(id) as id FROM sedmikostelibigramy WHERE word2=? AND word1=?");
