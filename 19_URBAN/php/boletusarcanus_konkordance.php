...
if (isset($_POST["submit"])){
    if ($_POST["boletusarcanus"]){
        ...
        $stmt = $conection->prepare("SELECT id, token FROM boletusarcanuskonkordance WHERE lemma=?");
        ...
        if ($rowcount > 0){
            while ($radek_lemma = $result_lemma->fetch_assoc()){
                ...
                if ($ID_LEMMA) {
                    ...
                    // PRAVÝ KONTEXT
                    $stmt2 = $conection->prepare("SELECT token FROM boletusarcanuskonkordance WHERE id=?");
