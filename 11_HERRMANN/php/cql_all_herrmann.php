...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (preg_match_all('/\[(\w+)\s*=\s*"([^"]+)"\]/', $query, $matches, PREG_SET_ORDER)) {
        ...
        if (!empty($conditions)) {
            $sql = "SELECT * FROM allherrmann WHERE " . implode(' OR ', $conditions) . " ORDER BY word ASC LIMIT ?";
            $stmt = $conn->prepare($sql);

            // Bind parameters dynamically
            $types = str_repeat('s', count($conditions)) . 'i';
            $params = array_merge(array_map(function($match) { return str_replace('*', '%', strtolower($match[2])); }, $matches), [$limit]);
