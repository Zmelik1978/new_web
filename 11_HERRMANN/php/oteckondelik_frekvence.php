<?php
// VYHLEDÁVNÍ af rf oteckondelik
if(isset($_POST["submit"])){
    if($lemma = $_POST["oteckondelik"]){
        $conection = mysqli_connect("127.0.0.1", "korpusprozyco001", "prasmoid", "korpusprozycom01");
        mysqli_set_charset($conection, "utf8mb4");
        if($conection){
            $sql = "SELECT af, rf FROM oteckondeliktags WHERE lemma='".$lemma."'";
    ...
    if($conect){
        $question = "SELECT DISTINCT LOWER (token) AS token FROM oteckondelikkonkordance WHERE lemma='".$lemma."'";
        ...
        if (mysqli_num_rows($query1) > 0){
            ...
            while($radek1 = mysqli_fetch_assoc($query1)){
                ...
                echo "<a href='frequency-list/oteckondelik.pdf' target='_blank'><img src='../../icons/file-earmark-pdf.svg' style='width: 20px; height: 20px; margin-left: 20px;'></a>";
            }
        } else {
            echo "<br><br>";
