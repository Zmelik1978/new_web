...
if (isset($_POST["submit"])){
    if ($_POST["concordance"]){
        ...
        $stmt = $conection->prepare("SELECT id, token, name, surname, title FROM allherrmannkonkordance WHERE lemma=? ORDER BY RAND()");
        $stmt->bind_param("s", $lemma);
        $stmt->execute();
        $result_lemma = $stmt->get_result();
        $rowcount = $result_lemma->num_rows; // počet řádků
        printf("<span style='color: blue; margin-left: -90px; font-size: 13px;'>Počet řádků: %d \n</span>", $rowcount);
        if ($rowcount > 0){
            while ($radek_lemma = $result_lemma->fetch_assoc()){
                ...
                if ($ID_LEMMA) {
                    ...
                    // PRAVÝ KONTEXT
                    $stmt2 = $conection->prepare("SELECT token FROM allherrmannkonkordance WHERE id=?");
                   ...
                   echo "<div style='float: left; width: 350px; text-align: left; font-size: 13px; color: brown;'>".$name." ".$surname.":<i>".$title."</i></div><br>";                   
            } 
        }
    } else {
        echo "<span style='color: red; margin-left: 500px; margin-top: 100px;'></span>";
    }
}  
}
?>
