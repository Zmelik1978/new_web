...
if(isset($_POST["submit"])){
    if($lemma = $_POST["tchankondelik"]){
        ...
        if($conection){
            $stmt = $conection->prepare("SELECT af, rf FROM tchankondeliktags WHERE lemma=?");
...
if($conect){
    $stmt = $conect->prepare("SELECT DISTINCT LOWER(token) AS token FROM tchankondelikkonkordance WHERE lemma=?");
    $stmt->bind_param("s", $lemma);
    $stmt->execute();
    $result = $stmt->get_result();
