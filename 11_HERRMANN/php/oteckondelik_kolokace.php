...
if(isset($_POST["submit"])){
    if($lemma = $_POST["oteckondelik"]){
        ...
        if($conection){
            $stmt = $conection->prepare("SELECT af, rf FROM oteckondeliktags WHERE lemma=?");
...
if($conect){
    $stmt = $conect->prepare("SELECT DISTINCT LOWER(token) AS token FROM oteckondelikkonkordance WHERE lemma=?");
    $stmt->bind_param("s", $lemma);
    $stmt->execute();
    $result = $stmt->get_result();
