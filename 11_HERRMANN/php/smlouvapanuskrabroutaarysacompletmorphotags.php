<?php
    ...
    if (isset($_POST["submit"])){
        if($tag = $_POST["smlouvapanuskrabroutaarysa"]){
            $conection1 = mysqli_connect("127.0.0.1","korpusprozyco001","prasmoid","korpusprozycom03");
            mysqli_set_charset($conection1, "utf8mb4");
            if($conection1){
                
                $sqltoken = "SELECT DISTINCT token, Af, Rf FROM smlouvapanuskrabroutaarysacompletmorphotags WHERE tag='".$tag."'";
    ...
    if (isset($_POST["submit"])){
        if($tag = $_POST["oteckondelik"]){
            $conection1 = mysqli_connect("127.0.0.1","korpusprozyco001","prasmoid","korpusprozycom03");
            mysqli_set_charset($conection1, "utf8mb4");
            if($conection1){
                
                $sqltoken = "SELECT DISTINCT token, Af, Rf FROM oteckondelikcompletmorphotags WHERE tag='".$tag."'";
                $resulttoken = mysqli_query($conection1, $sqltoken);
                $num_rows = mysqli_num_rows($resulttoken);
                if(mysqli_num_rows($resulttoken) > 0){
                    echo "<div style='width: 800px;'>";
