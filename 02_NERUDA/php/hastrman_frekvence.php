...
if(isset($_POST["submit"])){
    if($lemma = $_POST["hastrman"]){
        $conection = mysqli_connect("127.0.0.1", "korpusprozyco001", "prasmoid", "korpusprozycom01");
        mysqli_set_charset($conection, "utf8mb4");
        if($conection){
            $stmt = $conection->prepare("SELECT af, rf FROM hastrmantags WHERE lemma=?");
    ...
    if($conect){
        $stmt1 = $conect->prepare("SELECT DISTINCT LOWER(token) AS token FROM hastrmankonkordance WHERE lemma=?");
        ...
        if ($query1->num_rows > 0){
            ...
            while($radek1 = $query1->fetch_assoc()){
                ...
                echo "<a href='frequency-list/hastrman.pdf' target='_blank'><img src='../../icons/file-earmark-pdf.svg' style='width: 20px; height: 20px; margin-left: 20px;'></a>";
            }
        } else {
            echo "<br><br>";
