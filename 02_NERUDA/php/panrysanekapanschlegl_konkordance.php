...
if (isset($_POST["submit"])){
    if ($_POST["panrysanekapanschlegl"]){
        $lemma = $_POST["panrysanekapanschlegl"];
        $conection = mysqli_connect("127.0.0.1", "korpusprozyco001", "prasmoid", "korpusprozycom");
        mysqli_set_charset($conection, "utf8mb4");

        $stmt = $conection->prepare("SELECT id, token FROM panrysanekapanschleglkonkordance WHERE lemma=?");
        ...
        if ($rowcount > 0){
            while ($radek_lemma = $result_lemma->fetch_assoc()){
                ...
                if ($ID_LEMMA) {
                    ...
                    // PRAVÝ KONTEXT
                    $stmt2 = $conection->prepare("SELECT token FROM panrysanekapanschleglkonkordance WHERE id=?");
