...
if (isset($_POST["submit"])){
    if ($_POST["concordance"]){
        ...
        $stmt = $conection->prepare("SELECT id, token, name, surname, title FROM allstancikkonkordance WHERE lemma=? ORDER BY RAND()");
        ...
        if ($rowcount > 0){
            while ($radek_lemma = $result_lemma->fetch_assoc()){
                ...
                if ($ID_LEMMA) {
                    ...
                    // PRAVÝ KONTEXT
                    $stmt2 = $conection->prepare("SELECT token FROM allstancikkonkordance WHERE id=?");
