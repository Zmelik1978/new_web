...
if(isset($_POST["submit"])){
    if($lemma = $_POST["skapulir"]){
        $conection = mysqli_connect("127.0.0.1", "korpusprozyco001", "prasmoid", "korpusprozycom01");
        mysqli_set_charset($conection, "utf8mb4");
        if($conection){
            $stmt = $conection->prepare("SELECT af, rf FROM skapulirtags WHERE lemma=?");
...
if($conect){
    $stmt = $conect->prepare("SELECT DISTINCT LOWER(token) AS token FROM skapulirkonkordance WHERE lemma=?");
    $stmt->bind_param("s", $lemma);
    $stmt->execute();
    $result = $stmt->get_result();
    if ($result->num_rows > 0){
        ...
        echo "<a href='frequency-list/skapulir.pdf' target='_blank'><img src='../../icons/file-earmark-pdf.svg' style='width: 20px; height: 20px; margin-left: 20px;'></a>";
    } else {
        echo "<br><br>";
